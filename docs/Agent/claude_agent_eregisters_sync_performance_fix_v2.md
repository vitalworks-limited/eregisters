...
# Phase 17: Ensure Installed App Updates Take Effect in Already-Open Sessions

## Requirement

After a new eRegisters app version is built, uploaded, and installed in DHIS2, all users with already-open app sessions must receive the update automatically or be forced into a safe refresh flow. Users must not be required to manually clear cache, manually update the app, reinstall anything, or know technical deployment details.

This requirement is critical because the production performance fix must take effect quickly across all active browser sessions after deployment. Otherwise, users who opened the old app before deployment may continue running the old sync logic and continue generating heavy tracker traffic.

## Problem to Solve

DHIS2 custom apps are browser-loaded web applications. Existing browser sessions may continue using:

```text
cached JavaScript bundles
cached app shell files
cached manifest files
service worker caches
local storage state
IndexedDB state
already-loaded in-memory code
```

If users keep the app open after a new version is installed, they may continue running old sync logic such as:

```text
fields=*,enrollments[*,events[*]]
pageSize=100
async=false bulk tracker imports
immediate heavy sync on app open/login
unbounded full sync when updatedAfter is missing
```

The application must therefore include runtime version detection, safe update notification, cache busting, and automatic refresh behavior.

## Non-Negotiable Acceptance Criteria

```text
1. A newly deployed eRegisters app version must be detectable by already-open sessions.
2. Already-open sessions must stop starting heavy sync using old code after a new version is detected.
3. Users must not need to manually clear browser cache.
4. Users must not need to manually reinstall or manually update the app.
5. The app must safely refresh without losing unsaved form data.
6. If unsaved local form data exists, the app must save/draft it first or prompt the user before refresh.
7. Service worker and browser caches must not keep serving stale app bundles after deployment.
8. The app must expose its current version/build hash in diagnostics.
9. The app must support cache-busting for index.html, JS, CSS, manifest, app config, and version metadata.
10. The deployed DHIS2 app must use a unique version/build identifier for every release.
11. Old open sessions must not continue making the old expensive tracker sync requests indefinitely.
```

## Required Implementation

### 17.1 Add Build Version Metadata

Create a build/version module:
...
```ts
export const APP_NAME = "eregisters";
export const APP_VERSION = import.meta.env.VITE_APP_VERSION ?? "dev";
export const BUILD_HASH = import.meta.env.VITE_BUILD_HASH ?? "local";
export const BUILD_TIME = import.meta.env.VITE_BUILD_TIME ?? new Date().toISOString();
```

During build, inject values similar to:
...
If the project uses a different bundler or environment variable format, adapt accordingly.

### 17.2 Generate a Public Version File
...
or the equivalent static asset packaged with the DHIS2 app.
...
```json
{
  ...
  "version": "1.1.4",
  "buildHash": "abcdef1",
  "buildTime": "2026-06-25T20:00:00Z"
}
```

This file must be requested with cache busting:
...
If DHIS2 serves the app from a nested app base URL, use a relative URL that works from `/api/apps/eregisters/index.html`.

### 17.3 Add Runtime Version Polling
...
```text
1. On app startup, read current APP_VERSION and BUILD_HASH.
2. Every 5–15 minutes, fetch version.json using cache: "no-store".
3. If remote buildHash differs from current buildHash, set updateAvailable=true.
4. Stop heavy background sync immediately when updateAvailable=true.
5. Trigger the safe refresh flow.
```
...
export async function checkForAppUpdate(): Promise<boolean> {
  ...
  return Boolean(remote.buildHash && remote.buildHash !== BUILD_HASH);
}
```

### 17.4 Safe Refresh Flow

When a new version is detected:

```text
1. Pause background sync.
2. Prevent new full/heavy sync jobs from starting.
3. Check whether the user has unsaved form data.
4. If no unsaved data exists, show a short message and auto-refresh.
5. If unsaved data exists, save draft/local data first, then refresh.
6. If draft save is not possible, show a blocking prompt asking the user to save before refresh.
```

Recommended message when safe to auto-refresh:
...
Recommended message when unsaved data exists:

```text
A new eRegisters version is available. Please save or discard the current form to apply the update safely.
```

### 17.5 Force Old Sync Code to Stop

Add an update guard before every heavy sync action:

```ts
if (await isUpdateAvailable()) {
  pauseSync("App update available");
  await startSafeRefreshFlow();
  return;
}
```
...
any scheduled sync worker
```

The old running app must not continue the old heavy sync logic after a new version is detected.

### 17.6 Service Worker and Cache Handling

Inspect whether the app registers a service worker, Workbox, PWA plugin, or custom offline cache.
...
```text
1. Ensure the service worker detects new build files.
2. Call skipWaiting only after safe refresh conditions are met.
3. Clear old app-shell caches after update activation.
4. Keep clinical/offline data caches separate from app-shell caches.
5. Never delete IndexedDB clinical data during app update.
```

Important:

```text
Do not clear all browser storage.
Do not delete IndexedDB clinical/offline records.
Only clear app-shell/static asset caches.
```

### 17.7 Cache-Control Recommendations

Ensure deployed assets behave as follows:
...
unhashed JS/CSS assets: no-cache
hashed JS/CSS assets: long cache allowed
large static reference files: versioned URL or strong cache validation
```

If the app build currently emits unhashed assets, update the build to emit hashed assets where possible.

Expected behavior:

```text
New deployment changes buildHash.
Open sessions detect new buildHash.
Open sessions pause old sync.
Open sessions refresh safely.
After refresh, new app code runs.
```

### 17.8 DHIS2 App Versioning

Update app manifest version during release.
...
The version must change for every production release.
...
Do not redeploy a different build using the same version/build hash.

### 17.9 Diagnostics

Add current app version/build info to sync diagnostics:
...
Add visible support information somewhere appropriate:

```text
eRegisters version: 1.1.4
Build: abcdef1
Build time: 2026-06-25T20:00:00Z
```
...
```text
1. version.json is fetched with cache busting.
2. updateAvailable becomes true when buildHash changes.
3. heavy sync is blocked when updateAvailable=true.
4. safe refresh starts when no unsaved data exists.
5. refresh is delayed when unsaved form data exists.
6. IndexedDB clinical data is not cleared during update.
7. service worker cache cleanup does not delete local clinical/offline data.
8. diagnostics include appVersion/buildHash/buildTime.
9. old open sessions do not continue background sync after update detection.
```

### 17.11 Deployment Validation

After installing the fixed app in DHIS2:

```text
1. Open old app in browser A.
2. Deploy new app version to DHIS2.
3. Keep browser A open.
4. Confirm browser A detects update within 5–15 minutes.
5. Confirm browser A pauses old sync.
6. Confirm browser A refreshes safely.
7. Confirm browser A runs the new code after refresh.
8. Confirm no manual cache clear is needed.
9. Confirm local unsaved/offline data is preserved.
```

### 17.12 Server-Side Verification After Deployment

After deployment, production access logs must show the old expensive pattern disappearing:

```text
fields=*,enrollments[*,events[*]] should drop near zero.
pageSize=100 should drop near zero.
async=false bulk tracker POSTs should drop sharply.
```

Also verify that old open sessions are no longer making old sync calls after the update detection window.

## Final Acceptance Criteria for This Phase

```text
A user who opened eRegisters before deployment must automatically receive the new installed version without manual action.
The old open session must not continue running old sync logic indefinitely.
The update flow must not lose unsaved or offline clinical data.
The app must visibly report its version/build for support verification.
```
